...
(() => {
    ...
    // Advanced is loaded after app.js, but production must fail closed if a
    ...
    async function loadAdvanced(force = false, quiet = false) {
        if (!appState().weather)
            return;
        // suite.js is the canonical advanced engine in the production bundle.
    ...
    async function setAdvancedRadarLayer(layer) {
        ...
        if (layer === 'lightning') {
            if (SERVICES.get('suite')?.showLiveLightning) {
                return SERVICES.get('suite').showLiveLightning();
            }
    ...
    function bind() {
        ...
        qa('[data-advanced-radar-layer]').forEach(button => button.addEventListener('click', () => setAdvancedRadarLayer(button.dataset.advancedRadarLayer)));
