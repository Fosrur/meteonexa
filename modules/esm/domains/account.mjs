...
export function install(services, host = globalThis) {
    return services.installModule({
        ...
        (() => {
            ...
            // accountDomain.create() runs later from js/app.js, so publish a stable facade
            ...
            function create(context) {
                const { state, storage: STORAGE, saveJSON, loadJSON } = context;
                const isGuestSession = (...args) => context.isGuestSession(...args);
                const hasVerifiedServerSession = (...args) => context.hasVerifiedServerSession(...args);
                const apiRequest = (...args) => context.apiRequest(...args);
                const uniqueLocations = (...args) => context.uniqueLocations(...args);
                const normalizeLocation = (...args) => context.normalizeLocation(...args);
                const syncFavoriteUI = (...args) => context.syncFavoriteUI(...args);
                const renderFavorites = (...args) => context.renderFavorites(...args);
                const updateSelectedLocationUI = (...args) => context.updateSelectedLocationUI(...args);
                const meteonexaText = (...args) => context.meteonexaText(...args);
                ...
                function accountSyncDeviceId() {
                    const value = String(deps.security?.deviceId || '').trim();
                    return /^[A-Za-z0-9._-]{12,128}$/.test(value) ? value : '';
                }
                function accountSyncDeviceReady() { return accountSyncDeviceId() !== ''; }
                ...
                async function pushAccountFavorites() {
                    if (isGuestSession() || !hasVerifiedServerSession() || !accountSyncDeviceReady()) return null;
                ...
                function scheduleAccountFavoritesPush(delay = 420) {
                    if (isGuestSession() || !hasVerifiedServerSession() || !accountSyncDeviceReady()) return;
                ...
                async function pullAccountSync({ force = false } = {}) {
                    if (isGuestSession() || !hasVerifiedServerSession() || !accountSyncDeviceReady() || accountSyncState.loading || (accountSyncState.loaded && !force)) return null;
                ...
                async function saveAccountActivityProfile(activity, thresholds) {
                    if (isGuestSession() || !hasVerifiedServerSession() || !accountSyncDeviceReady()) throw new Error(meteonexaText('api.security.auth_required'));
                ...
                async function hydrateAuthenticatedAccountAfterLogin({ adoptSavedLocation = true } = {}) {
                    if (!hasVerifiedServerSession() || isGuestSession() || !accountSyncDeviceReady()) return null;
