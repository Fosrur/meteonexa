# MeteoNexa 20.1 RC2 — Documentazione consolidata
...
Questa è l’unica documentazione Markdown della release. Consolida README, architettura, security, audit RC, release notes, checklist e note CSS precedentemente separate.

### Fix applicato in questa revisione

Errore production osservato:

- `Uncaught ReferenceError: updateThreshold is not defined` in `app.js`;
- successivi `METEONEXA_RUNTIME_API_NOT_READY` in `advanced.js`, `suite.js` e bootstrap ESM.

Causa radice: `updateThreshold()` era stato estratto in `modules/esm/domains/app-utilities.mjs`, ma il servizio `appUtilities` non lo restituiva e `app.js` non lo acquisiva tramite destructuring. Quando `app.js` costruiva il payload per `runtimeApi.publish(...)`, la reference inesistente interrompeva l’esecuzione prima della pubblicazione della Runtime API. I bundle Advanced/Suite, correttamente dipendenti da `runtimeApi.get()`, fallivano quindi a cascata con `METEONEXA_RUNTIME_API_NOT_READY`.

Correzioni:

- `appUtilities.create()` ora espone `updateThreshold`;
- `app.js` lega `updateThreshold` dal servizio prima di pubblicare la Runtime API;
...
- asset fingerprintati rigenerati (`app.js` -> `dist/app.b3d4fa62ce8b.js`, `app-utilities.mjs` -> `dist/modules/esm/domains/app-utilities.91c064bdb67d.mjs`);
...
La causa del bug non richiede rollback dell’architettura ESM/DI. Il problema era un contratto di export incompleto al confine tra `appUtilities` e il classic shell `app.js`. Il fix mantiene la direzione architetturale: singolo state autorevole, service registry chiuso, Runtime API pubblicata solo dopo la composizione del main shell, domini ESM con dipendenze dichiarate. Il nuovo test di regressione verifica esattamente quel confine.
...
- Dominio produzione canonico: **https://meteonexa.com/**; le navigazioni HTML su **https://www.meteonexa.com/** ricevono redirect **308** verso l’apex.
...
- catalogo: **4.656 chiavi × 5 lingue = 23.280 traduzioni**;
...
Cookie Policy e implementazione restano allineate ai cookie tecnici first-party (`meteonexa_auth_session`, `meteonexa_trusted_device`, `meteonexa_client`, `meteonexa_language`) e al pageview analytics Plausible cookieless. Nessun cookie pubblicitario/profilante è introdotto da questa tranche.
...
L'audit della RC ha rilevato un problema non intercettato dai precedenti smoke test: `visualization.mjs` usava ancora un identificatore `SERVICES` non dichiarato quando costruiva il controller radar e `app.js` utilizzava funzioni radar non restituite dalla factory. In browser questo poteva produrre `ReferenceError` durante il bootstrap o all'apertura Radar. La RC corregge il contratto: `visualization` dichiara `radarMotion` e `radarController` nel dependency graph, usa `deps.*` e restituisce esplicitamente le funzioni radar consumate da `app.js`. Il gate `qa/release_candidate_smoke.py` impedisce la regressione.

### Critical path più leggero

- i moduli esclusivi di Suite (`suite-support`, `route-weather`, `suite-assistant`) non fanno più parte del gruppo `PRE_APP_ESM`: vengono importati/installati solo dopo `app.js` e Copilot, prima di `suite.js`;
- `app.js` viene pre-caricato ad alta priorità mentre Core e domini ESM vengono installati;
- `suite.css`, `intelligence.css`, `decision-timeline.css`, `watch-plan.css`, `copilot.css` e MapLibre CSS non bloccano più il first paint: vengono applicati dopo il primo frame;
...
- 4.656 chiavi × 5 lingue = **23.280 traduzioni**;
- chiavi i18n hash-like attive: **0**;
...
Dopo questa tranche `app.js` passa da circa **565 KB / 10.668 righe** del primo P1 a circa **507 KB / 9.506 righe**. I nuovi moduli sono sorgenti first-party fingerprintate in `dist/`; il caricamento resta `defer` e avviene prima di `app.js`, quindi non vengono introdotti bundle/runtime esterni.

### P1 fase 3 — controller radar, MapLibre e playback

La terza tranche P1 completa la separazione del controller radar principale da `app.js`. Il nuovo `modules/esm/domains/radar-controller.mjs` possiede il lifecycle della mappa MapLibre, il layer raster live, fallback canvas, recovery WebGL, sincronizzazione camera, ricerca/GPS dalla pagina radar, caricamento live/forecast, refresh, cambio modalità, frame slider, playback e zoom. Il motore predittivo resta separato in `modules/esm/domains/radar-motion.mjs`: controller e motion condividono lo stesso `state` runtime ma hanno responsabilità distinte.

Il controller non introduce un nuovo store o nuove API globali applicative: viene creato dal bootstrap con dipendenze esplicite (`CONFIG`, utility DOM, persistenza, ricerca località, fetch meteo e funzioni del motion engine). `app.js` conserva solo il wiring tra i moduli e le chiamate usate dal resto dell'interfaccia. Anche `radar-controller.js` è incluso nel fingerprinting `dist/` e viene caricato con `defer` prima di `app.js`.

Dopo questa tranche `app.js` scende da circa **507 KB / 9.506 righe** a circa **461 KB / 8.429 righe**. Il blocco radar estratto misura circa **57 KB / 1.121 righe**, ma è ora isolato per dominio e può essere testato/refactorizzato senza attraversare il bootstrap generale. Versione applicazione, schema DB, endpoint e comportamento utente restano invariati.

### P1 fase 4 — località, ricerca, preferiti e navigazione

La quarta tranche P1 estrae da `app.js` il dominio località in `modules/esm/domains/locations.mjs`. Il modulo gestisce geolocalizzazione browser e reverse geocoding, ricerca città, selezione onboarding, recenti, cambio località, preferiti, caricamento sintetico delle card preferite e le azioni di ricerca globale/command search. Il bootstrap continua a fornire dipendenze esplicite e lo stesso `state` runtime: non viene introdotto un secondo store e gli endpoint/provider restano invariati.

`modules/esm/domains/navigation.mjs` non è più soltanto l'event bus per `navigation-request`: possiede ora anche la policy guest/auth, la visibilità delle feature/pagine, il caricamento della configurazione UI, il routing tra pagine e il relativo lifecycle. `app.js` conserva il wiring verso radar, history, Intelligence e Advanced mediante callback esplicite, evitando dipendenze implicite o un service locator globale.

Dopo questa tranche `app.js` scende a circa **430 KB / 7.886 righe**. Geolocalizzazione, ricerca/preferiti e routing non sono più implementati nel bootstrap principale. `locations.js` e il controller esteso di `navigation.js` sono inclusi nel fingerprinting `dist/` e caricati con `defer` prima di `app.js`. Resta un ultimo blocco P1 concentrato su notification center/push e pulizia finale del bootstrap; dopo quel passaggio il progetto può entrare nel P2 senza lasciare i principali domini funzionali dentro `app.js`.
...
Il nuovo `meteonexa_migration_manifest()` rende esplicita la sequenza schema **16 → 26** e `meteonexa_current_schema_version()` è l’unica costante runtime usata dal bootstrap per la versione corrente. Le migration storiche sono state tolte dalla funzione di connessione; le future revisioni DB devono essere aggiunte al modulo migration invece di far crescere nuovamente `meteonexa_db()`. La baseline SQLite e `mysql-schema.sql` restano gli snapshot completi per nuove installazioni.

### P1 finale — notification center, push e PWA

L'ultima tranche P1 estrae da `app.js` il dominio notifiche in `modules/esm/domains/notifications.mjs`. Il modulo possiede ora capability detection del dispositivo/browser, stato permessi, UI del Notification Center, inbox server tramite `api/push/inbox.php`, badge applicazione, notifiche meteo locali, invio tramite Service Worker/Notifications API e preferenze severe/rain. Anche la card degli avvisi ufficiali esposta nel Notification Center viene coordinata dal dominio tramite callback esplicite verso il sottosistema Alerts.
...
Dopo il P1 finale `app.js` scende a circa **406 KB / 7.480 righe**, rispetto ai circa **598 KB / 11.303 righe** dell'inizio dell'audit. `modules/esm/domains/notifications.mjs` è un asset first-party fingerprintato e caricato con `defer` prima di `app.js`; non sono stati introdotti nuovi endpoint, provider, store o dipendenze runtime esterne.
...
`qa/p1_maintainability_smoke.py` protegge il refactoring verificando: assenza del vecchio bridge frontend, presenza dei nuovi moduli fingerprintati, facade DB sottile, separazione dei sottosistemi database, manifest migration continuo fino allo schema 26 e documentazione README aggiornata. I test preesistenti che ispezionavano direttamente `api/database.php` sono stati adattati a leggere i moduli reali senza indebolire i controlli.

## 20.1 — P2 fase 1: toolchain e confini runtime espliciti

Il P2 parte dalla base P1 senza cambiare **versione 20.1, schema 26, API, provider, UI o comportamento funzionale**. Il primo obiettivo non è convertire in blocco l'applicazione a un framework o a ES modules, ma rimuovere le dipendenze JavaScript implicite che renderebbero rischiosa quella conversione.

### Service registry

`modules/esm/core/service-registry.mjs` è il boundary usato da `app.js` per risolvere i servizi `MeteoNexa*`; nella fase 1 lo stesso contratto era ancora uno script classico e dalla fase 2 è un ES module nativo. Il registry espone `get`, `require`, `publish` e una allowlist chiusa di nomi di servizio. `app.js` non accede più direttamente a decine di `window.MeteoNexa...`: le dipendenze sono nominate, verificabili e falliscono esplicitamente quando un servizio obbligatorio non è ancora disponibile. I globali compatibili restano temporaneamente pubblicati perché altri moduli legacy li consumano ancora; il registry è quindi un **compatibility boundary**, non un secondo container di stato.

### Runtime API tra bootstrap, Advanced e Suite

`modules/esm/core/runtime-api.mjs` definisce il contratto esplicito tra il bootstrap principale e i bundle legacy caricati dopo `app.js`; dalla fase 2 il contratto è esportato/importato come ES module nativo. `app.js` pubblica una API immutabile con accesso allo stato autorevole e agli helper condivisi necessari (`loadJSON`, `saveJSON`, loader, weather refresh, threshold, locale/formattazione, ecc.). `advanced.js` e i due domini principali di `suite.js` non dipendono più dal fatto che dichiarazioni top-level di uno script classico diventino accidentalmente proprietà globali del browser.
...
Il comando `npm run check` usa `tools/static-analysis.mjs` e non richiede pacchetti npm esterni: usa Node >= 20, esegue `node --check` su JS/MJS, vieta `eval`, `new Function` e `document.write`, verifica che `app.js` passi dal service registry e protegge il contratto Runtime API di Advanced/Suite. Questo mantiene il source package riproducibile anche in ambienti CI/offline; ESLint/esbuild potranno essere aggiunti in una tranche P2 successiva quando il boundary ESM sarà sufficientemente maturo.
...
`qa/p2_toolchain_smoke.py` verifica package/toolchain, ordine di caricamento, fingerprinting dei nuovi boundary, assenza di bypass `window.MeteoNexa*` in `app.js`, uso esplicito della Runtime API in Advanced/Suite e documentazione P2. `qa/run-all.sh` esegue inoltre `tools/static-analysis.mjs` come gate di release.
...
`modules/esm/bootstrap.mjs` è caricato con `type="module"`. Legge il manifest first-party `asset-manifest.js`, importa i boundary ESM tramite URL fingerprintati e, dalla fase 3, installa anche l'intero core applicativo ESM prima di caricare in sequenza i domini P1 ancora classici. Solo dopo avvia `app.js`, `advanced.js`, Copilot, `suite.js`, Weather Intelligence e i feature controller successivi. In questo modo il passaggio a ESM non dipende più dall'ordine temporale implicito tra script `defer` e module script.

`app.js`, `advanced.js` e il date picker di `suite.js` sono inoltre **late-load safe**: se `DOMContentLoaded` è già avvenuto avviano il proprio bind tramite `queueMicrotask`; se il documento è ancora in parsing mantengono il listener one-shot. Questo rende il bootstrap robusto anche a futuri cambi di bundling/preload.
...
`modules/esm/bootstrap.mjs` è ora l'autorità sull'ordine di inizializzazione. Importa in parallelo i sette moduli core ESM fingerprintati, installa service registry/Runtime API/core e poi carica in sequenza `PRE_APP_CLASSIC_SCRIPTS` (domini, Route Weather, custom controls, Product Metrics e Analytics). Solo quando i contratti pre-app sono disponibili carica il tail applicativo (`app.js`, Advanced, Copilot, Suite, Weather Intelligence e feature controller). `index.html` conserva soltanto i runtime di base (`i18n-runtime`, config, security), `asset-manifest.js` e il bootstrap `type="module"`.
...
Il P2 è completato. Tutti i moduli applicativi sotto `modules/domains/` e `modules/features/` sono stati rimossi e sostituiti dalle implementazioni autorevoli sotto `modules/esm/domains/` e `modules/esm/features/`. Il bootstrap importa/installla i domini ESM prima di `app.js` e installa le feature ESM nel medesimo ordine funzionale della pipeline precedente, senza duplicare sorgenti classic.

`MeteoNexaServices` è ora un **registro interno reale**: `publish()` memorizza il servizio nel registry e non crea più automaticamente `window.MeteoNexa*`. L'eventuale esposizione legacy è separata ed esplicita tramite `exposeLegacy()`. Core, Runtime API, domini e feature vengono quindi risolti dal registry; `app.js`, `advanced.js`, `suite.js`, Weather Intelligence, Product Metrics e Custom Controls usano tutti il solo boundary `window.MeteoNexaServices`. I runtime foundational ancora classic (`MeteoNexaSecurity` e `MeteoNexaI18n`) restano temporaneamente first-party global perché sono caricati prima del bootstrap ESM; Analytics mantiene un fallback globale solo sulle pagine standalone che non caricano il registry.
...
Il Service Worker usa la shell revision `p2-esm-services-3` e precachea core, domini e feature ESM fingerprintati. `tools/esbuild-verify.mjs` include l'intero grafo ESM come superficie di verifica. La QA aggiunge `qa/p2_domain_esm_smoke.mjs`, che importa tutti i domini/feature con Node e verifica che espongano `install()`/`serviceNames` senza creare globali. `tools/static-analysis.mjs` impedisce inoltre a `app.js`, Advanced, Suite, Weather Intelligence, Product Metrics e Custom Controls di bypassare il service registry.

**Stato P2: COMPLETATO.** Il passo successivo può essere P3; non è necessaria un'altra tranche P2 per il boundary runtime.

## 20.1 — Analytics & Observability

La 20.1 parte dalla 20.0 mobile hotfix già verificata e **non modifica il motore meteorologico, il Decision Engine o lo schema DB**: resta `schema 26 / 47 tabelle`. Introduce Plausible Analytics esclusivamente per statistiche di traffico aggregate e mantiene i Product Metrics first-party come pipeline separata per l’uso delle funzioni.

L’integrazione non carica JavaScript di terze parti: `analytics.js` è un asset MeteoNexa fingerprintato e invia direttamente all’Events API ufficiale di Plausible soltanto eventi `pageview`. I percorsi vengono sanitizzati (`/home`, `/radar`, `/intelligence`, `/advanced`, `/route`, ecc.); coordinate, località selezionata, account/device ID, query di ricerca, testo AI, contenuto bug report e coordinate route non entrano nel payload. Sulla prima pageview possono essere mantenuti soltanto i parametri di acquisizione whitelistati (`utm_*`, `ref`, `source`) e il referrer viene ridotto alla sola origine.

Plausible è configurabile con `METEONEXA_PLAUSIBLE_ENABLED` e `METEONEXA_PLAUSIBLE_DOMAIN`; il default del pacchetto abilita il provider per `meteonexa.com`. Prima del deploy operativo va creato/registrato il site `meteonexa.com` nel dashboard Plausible; l'invio dei pageview all'Events API non richiede una Stats API key nel browser. L’endpoint è fisso a `https://plausible.io/api/event` e la CSP continua a vietare script esterni (`script-src 'self'`), estendendo solo `connect-src` a `https://plausible.io`. Un ad-blocker, timeout o configurazione Plausible assente non può bloccare login, guest, navigazione o meteo: il modulo è sempre fail-soft e non esegue retry invasivi.

Il Privacy Center aggiunge un controllo Analytics ON/OFF. L’opt-out viene ricordato soltanto nella preferenza locale `meteonexa_analytics_optout_v1`; Global Privacy Control e Do Not Track disabilitano automaticamente l’invio. L’avviso privacy resta informativo, non diventa un cookie wall. Privacy Policy e Cookie Policy distinguono chiaramente Plausible dai Product Metrics e documentano provider, categorie di dati, opt-out e assenza di cookie analytics.

Diagnostics aggiunge un controllo Plausible separato dal riepilogo Product Metrics. La suite QA contiene un gate `analytics_observability_smoke.py` che protegge: nessuno script Plausible esterno, URL sanitizzati, assenza di dimensioni sensibili, GPC/DNT, opt-out, CSP, policy/i18n, separazione Product Metrics e schema invariato. Il catalogo corrente è `translation_seed_version=20.1-semantic-i18n-v1`: **4.653 chiavi × 5 lingue = 23.265 traduzioni**.
...
La 20.0 mantiene **schema 26 / 47 tabelle**. Il catalogo corrente è `translation_seed_version=20.0-alignment-hardening-v3`: **4.623 chiavi × 5 lingue = 23.115 traduzioni**. Non vengono introdotti nuovi cookie, tracker o SDK analytics; Privacy e Cookie Policy espongono soltanto la release corrente.
...
La 19.5 non aggiunge provider, cookie, tracker, fingerprint, SDK analytics o nuove categorie di dati. Il catalogo corrente è `translation_seed_version=19.5-watch-my-plan-v1`: **4.666 chiavi × 5 lingue = 23.330 traduzioni**.

## 19.4 — Public Local Accuracy / Trust

La 19.4 mantiene **schema 26 / 47 tabelle** e rende pubblicabile, nello stesso article Accuracy esistente, una vista locale delle evidenze verificate degli ultimi 60 giorni. Non viene creato un secondo article: `#model-accuracy-panel` conserva lo stesso contratto `glass-panel / panel-title` e aggiunge un blocco interno responsive.
...
L'article **Decision Timeline** usa lo stesso contratto grafico `glass-panel / panel-title` degli article Intelligence esistenti e breakpoint dedicati a 720/460 px. Il pannello Forecast Change esistente viene aggiornato in-place, evitando duplicazioni visive. I nuovi asset sono modulari (`api/intelligence/v192_helpers.php`, `modules/esm/features/intelligence-v192.mjs`, `intelligence-v192.css`). Privacy Policy e Cookie Policy dichiarano esplicitamente che la 19.2 riusa dati e snapshot già trattati e non introduce cookie, tracker, storage analytics o fornitori esterni. Il catalogo corrente è `translation_seed_version=19.2-decision-timeline-forecast-change-v2-v1`: **4.561 chiavi × 5 lingue = 22.805 traduzioni**.

## 19.1 — Privacy-first Product Metrics

La 19.1 parte dalla 19.0.9 Reliability Consistency già verificata e introduce **Product Metrics first-party**, senza cambiare il motore meteorologico. Il contratto dati passa a **schema 26 / 47 tabelle** con `product_metrics_daily`: non esiste una riga per visita, utente, account, dispositivo o sessione. Il server conserva soltanto `metric_date + event_name + event_count + updated_at`, quindi lo storico è aggregato prima della persistenza.
...
La 19.0.8 mantiene **schema 25 / 46 tabelle** e corregge un problema di packaging della 19.0.7: `index.html` poteva conservare hash precedenti di `app.js`, `suite.js` e `weather-intelligence.js` mentre `asset-manifest.json` e `dist/` contenevano hash più recenti. In produzione le URL mancanti venivano quindi risolte sulla pagina HTML di errore (`text/html`); con `X-Content-Type-Options: nosniff` il browser bloccava correttamente quei file JavaScript e `advanced.js` poteva poi generare il secondario `state is not defined`.

Il builder `tools/fingerprint_assets.py` ora normalizza anche **riferimenti fingerprintati obsoleti non presenti nel manifest corrente**, quindi può riparare un work tree parzialmente ricostruito. Il nuovo gate `qa/release_19008_asset_contract.py` verifica sorgente → SHA-256 → nome fingerprint → contenuto `dist/` → manifest → riferimenti HTML/PHP. Un asset referenziato ma assente blocca la release. `advanced.js` usa inoltre un accesso resiliente allo stato legacy: se il bundle principale non è disponibile non produce più una cascata `ReferenceError`, lasciando visibile il guasto originario.
...
Il backend usava già sei famiglie (`ECMWF IFS`, `ECMWF AIFS AI`, `ICON`, `GFS`, `Météo-France`, `UKMO`), ma due percorsi legacy del browser e il renderer dei voti restavano a cinque. AIFS è ora presente anche in `config.js`, preview/client fetch, Suite e lista voti. Il kicker i18n è **“Consensus 6 modelli”**. Le etichette modello passano dalle chiavi `provider.*` invece di copy UI literal.
...
Le superfici HTML principali non contengono copy utente non tradotto; JavaScript/Service Worker non assegnano messaggi UI tramite literal prose. Brand, simboli, unità (`°C`, `km/h`, `mm`), codici tecnici (`DB`, `AI`, `HTTPS`, `SMTP`) e nomi propri dei provider non sono copy traducibile. Sono state corrette anche etichette generiche rimaste in inglese nelle lingue ES/FR/DE (Calibration Worker, servizi, destinazione/distanza, satellite, trend stabile, Intelligence, Smart Alerts, Weather Confidence e Personal Weather Twin). Il catalogo corrente è `19.0.6-production-radar3-active-i18n-security-v1`: **4.486 chiavi × 5 lingue = 22.430 traduzioni**. La migration usa hash SHA-256 dei valori package precedenti per aggiornare soltanto le copie release-owned, preservando eventuali personalizzazioni DB.
...
Le navigazioni HTML verso `www.meteonexa.com` vengono inoltre portate con redirect 308 a `https://meteonexa.com`, mentre API e Service Worker non vengono forzati cross-origin: questo consente alle vecchie PWA controllate su `www` di aggiornarsi senza spezzare il bootstrap e riduce progressivamente il doppio storage browser osservato in produzione. Dopo il deploy è consigliato usare sempre il dominio canonico `https://meteonexa.com/`.
...
Il catalogo corrente è `translation_seed_version=19.0.1-verified-contract-qa-access-v1` con **4.467 chiavi × 5 lingue = 22.335 traduzioni**. `privacy.html`, `cookie-policy.html`, `README.md`, `SECURITY.md` e `ARCHITECTURE.md` sono allineati alla 19.0.1. Il gate anti-hardcode verifica le superfici HTML principali e i moduli UI 19.x; brand, unità meteorologiche, codici tecnici e nomi propri dei provider non vengono trattati come copy traducibile.
...
**Stato P3 fase 1:** adapter duplicati eliminati e grafo dei servizi reso esplicito. La business logic dei domini è invariata; le fasi P3 successive possono sostituire progressivamente il compatibility window con injection/import diretti dominio per dominio e alleggerire ulteriormente `app.js`/`suite.js`.
...
**Stato P3 fase 2:** dependency injection diretta completata per domini e feature; il compatibility window non è più usato nel grafo applicativo ESM. Restano da alleggerire i grandi shell classici (`app.js`/Suite) e, dove conveniente, sostituire il service lookup con import/callback più locali.
...
Sono stati introdotti `modules/core/store.js` e moduli separati per `navigation`, `weather`, `radar`, `alerts`, `auth`, `privacy`, `feedback` e `ai`. Il nuovo store è la destinazione comune degli aggiornamenti; un bridge controllato mantiene la compatibilità con il legacy `state` mentre `app.js`/`suite.js` vengono svuotati progressivamente. Le feature 18.10.0 (`intelligence-v2` e `route-v3`) nascono già sul nuovo layer.

### Asset fingerprintati

`tools/fingerprint_assets.py` genera asset SHA-256 in `dist/` con hash a 12 caratteri e i manifest `asset-manifest.json` / `asset-manifest.js`. Le pagine pubbliche e il Service Worker usano gli asset fingerprintati, eliminando il rischio di servire JS/CSS vecchi con nomi stabili e cache lunga.
...
- aggiunto `favicon.ico` multipla risoluzione alla root;
...
- `/favicon.ico` non viene più mantenuto `immutable`: può essere rivalidato ad ogni release.
...
La separazione API PHP / PWA / worker / DB è sensata e l'app ha già un numero significativo di boundary server-side. Il principale debito tecnico è invece nel frontend: `app.js` è diventato molto grande e `advanced.js` / `suite.js` estendono diversi lifecycle globali. Questo aumenta il rischio di race condition, doppi listener e regressioni come quella del loader Radar.
...
- aggiunti `robots`, `googlebot`, canonical assoluto verso `https://meteonexa.com/`, Open Graph e Twitter Card;
...
Per massimizzare la canonicalizzazione è consigliato anche un redirect HTTP 301 lato Caddy da `www.meteonexa.com` a `meteonexa.com`; non viene imposto dal codice applicativo per non modificare il routing di produzione senza una scelta infrastrutturale esplicita.

### Tema chiaro

È stato introdotto `light-theme.css`, caricato **dopo** `styles.css`, `advanced.css` e `suite.css`. Il tema chiaro non è più una piccola sovrascrittura del tema scuro ma un layer visuale centralizzato con palette e contrasto coerenti.
...
- `standalone.css` è stato riallineato per Privacy, Cookie e Offline.
...
- Eliminato il flash iniziale del sole: il markup parte in stato `loading`, `boot-clock.js` risolve la scena dalla sola cache locale prima del primo paint utile e la UI viene resa visibile solo dopo la scelta della scena.
...
Per servire entrambi gli host mantenendo **meteonexa.com** come origin canonico, il reverse proxy può accettare entrambi gli host; l’applicazione canonicalizza le normali navigazioni HTML `www` con 308:
...
```bash
curl -I https://meteonexa.com/
curl -I https://www.meteonexa.com/
```

Entrambi devono arrivare all'app e riportare `via: 1.1 Caddy`; `www` non deve più rispondere con un redirect verso il dominio senza `www`.
...
La Privacy Policy non contiene più riferimenti al vecchio dominio o al vecchio brand. Il sito pubblico indicato è `meteonexa.com`; il contatto privacy viene letto dal deployment tramite `METEONEXA_PRIVACY_CONTACT_EMAIL`, con fallback al destinatario bug configurato.
...
[ ] meteonexa.com HTTP 200
[ ] www.meteonexa.com navigazione HTML -> 308 verso meteonexa.com
...
## 20.1 — P3 fase 3: decomposizione dei shell `app.js` e Suite

La terza tranche P3 riduce il codice ancora concentrato nei due shell classici senza cambiare il contratto funzionale. Il motore di visualizzazione (canvas setup, tooltip/interaction dei grafici, chart meteo, weather FX e grafici motion/trend) è ora autorevole in `modules/esm/domains/visualization.mjs`; `app.js` conserva soltanto il wiring esplicito del contesto e le chiamate ai metodi restituiti dalla factory. Questo porta `app.js` da circa **7.509 a 6.493 righe**, eliminando oltre mille righe di implementazione grafica dal bootstrap principale.

Suite delega invece API constants, storage keys, HTTP JSON wrapper, localizzazione/formattazione comune e helper data/geografici a `modules/esm/domains/suite-support.mjs`. `suite.js` passa da circa **3.066 a 2.946 righe** e rimane focalizzato sui flussi funzionali (modelli, route, assistant e UI della suite) invece di duplicare infrastruttura di base.
...
**Stato P3 fase 3:** shell ulteriormente alleggeriti e responsabilità grafiche/runtime condivise spostate in ESM con boundary espliciti. Le prossime tranche possono concentrarsi sui grandi blocchi funzionali ancora presenti in `app.js` (forecast/intelligence/history/device lifecycle) e sulla separazione dei controller Suite.

## 20.1 — P3 fase 4: forecast/history/intelligence, lifecycle e integrazioni Suite

La quarta tranche P3 separa dai shell classici altri blocchi funzionali ad alta responsabilità, mantenendo invariati API pubbliche, stato runtime e flussi UI. `app.js` passa da circa **6.493 a 4.940 righe** e `suite.js` da circa **2.946 a 2.307 righe**.
...
I shell scendono da circa **4.940 a 4.637 righe per `app.js`** e da circa **2.307 a 1.343 righe per `suite.js`**. `qa/p3_shell_decomposition_smoke.mjs` impone ora budget permanenti di **4.700** e **1.400** righe e rifiuta la reintroduzione delle implementazioni estratte. Il grafo applicativo comprende **28 moduli domain/feature ESM** e almeno **31 servizi dichiarati**; la revisione PWA corrente è `p3-final-shells-5`.
...
### Stato della roadmap originale (aggiornato fino a P4 fase 3)
...
| P1 | spezzare progressivamente `app.js` | **COMPLETATO** (shell sotto 4.700 righe) |
...
| P2 | riorganizzare CSS | **COMPLETATO in P4 fase 3**: `styles.css` e `suite.css` sono artifact generati da partial ordinati con cascade byte-equivalent e gate dedicato |
...
Il job MySQL 8.4 esegue ora sia `qa/mysql_auth_integration.php` sia `qa/mysql_full_integration.php`: quest'ultimo ricrea lo schema reale, verifica le 47 tabelle, attraversa la catena di upgrade da schema 15 fino allo schema 27 e applica i capability sync idempotenti e verifica i trigger di revisione traduzioni.
...
La seconda tranche P4 chiude i due gap backend rimasti nella roadmap originale senza modificare, in quella tranche, lo schema **26**, le API pubbliche o il comportamento applicativo. Il P4 finale introduce successivamente lo schema 27 esclusivamente per la migrazione delle chiavi i18n.
...
Ogni revisione dichiara `version`, `name`, driver supportati e callback `up`. Il registry rifiuta revisioni mancanti/non consecutive, driver sconosciuti e mismatch tra l'ultima migration e `meteonexa_current_schema_version()`. MySQL attraversa il registry 16→27; SQLite conserva `legacy_sqlite_upgrade.php` esclusivamente come adapter quarantinato per database creati prima del contratto per-revisione. Le revisioni future devono essere file autonomi e, dalla revisione 27 in avanti vengono applicate anche a SQLite dal runner esplicito.
...
La terza tranche P4 chiude il gap CSS della roadmap originale senza modificare l'output visuale. I due fogli legacy più grandi restano disponibili con gli stessi nomi pubblici (`styles.css` e `suite.css`) per compatibilità con fingerprinting, Service Worker e test, ma non sono più la sorgente da modificare direttamente.
...
`qa/p4_css_architecture_smoke.py` verifica l'equivalenza aggregate↔partials, l'assenza di `@import` runtime, l'ordine deterministico, il wiring del build e la documentazione. Il comando dedicato è `npm run check:p4:css`. I fogli feature già piccoli (`advanced.css`, `intelligence.css`, `light-theme.css`, ecc.) restano indipendenti e non vengono inglobati artificialmente nel bundle principale.
...
L'audit della roadmap originale è stato ripetuto sul codice corrente, non sulle sole etichette dei milestone. Il risultato è: **P0 completo, P1 completo, P3 completo; in P2 restano due soli gap reali**.

| Priorità originale | Voce | Audit corrente |
| --- | --- | --- |
| P0 | admin QA hardcoded / SMTP→admin | **COMPLETO** |
| P0 | pin/hash MapLibre | **COMPLETO** |
| P1 | decomposizione `app.js` | **COMPLETO** |
...
Durante l'audit è stato corretto anche `api/system/status.php`: il controllo health non confronta più lo schema con il valore storico `25`, ma con `meteonexa_current_schema_version()` (**27** corrente), e il failure DB espone ora `DATABASE_UNAVAILABLE` con chiave messaggio stabile.
...
Le chiavi legacy nel formato `ui.<hash>` / `code.<hash>` sono state eliminate dai cataloghi attivi. Le **4.653 chiavi** correnti restano complete in **5 lingue** (**23.265 traduzioni**), ma le 1.221 chiavi opache sono ora leggibili e organizzate per dominio/contesto. La generazione è deterministica e usa dominio runtime, funzione/elemento proprietario e copy inglese come base del nome.

La compatibilità con installazioni esistenti è gestita da `api/install/i18n-key-map-20.1.json`, che contiene esattamente le **1.221** corrispondenze old→new. La migration `api/database/migrations/0027_semantic_i18n_keys.php` rinomina in-place le righe `translations` su MySQL e SQLite, preservando traduzioni eventualmente personalizzate dagli operatori. Lo schema corrente è quindi **27** e `translation_seed_version` è `20.1-semantic-i18n-v1`.
...
| P0 | eliminare admin QA hardcoded e fallback SMTP→admin | **COMPLETATO** |
| P0 | pin/hash asset JS terzi MapLibre | **COMPLETATO** |
| P1 | spezzare progressivamente `app.js` | **COMPLETATO** — ~11.303 → 4.637 righe |
| P1 | un solo stato frontend / rimuovere bridge legacy | **COMPLETATO** |
| P1 | spezzare `database.php` | **COMPLETATO** |
...
Product Metrics is first-party and aggregate-only. The client keeps short-lived counters in memory, submits a closed event allowlist with `credentials: omit`, and the server stores only date, event name and aggregate count. No email, device identifier, precise location, AI prompt/body or full location name is accepted by the metrics endpoint. Retention is capped and a deployment kill switch is available.

## Traffic Analytics and observability

Traffic analytics is deliberately separate from Product Metrics. `analytics.js` is a same-origin, fingerprinted MeteoNexa asset; no third-party JavaScript is executed. It fetches a public, non-secret configuration from `api/analytics/config.php` and sends only `pageview` events to the fixed Plausible Events API origin. Application navigation is converted to an allowlisted semantic path and never includes selected locations, coordinates, search text, AI content, route samples, account/device identifiers or application payloads. Only allowlisted acquisition parameters may be preserved on the initial pageview and external referrers are reduced to their origin.

The analytics module is fail-soft and outside the boot/login critical path. Network failure, blocking extensions or a missing Plausible site configuration cannot hold a loader or change application state. GPC/DNT and the local Privacy Center opt-out disable dispatch. The opt-out preference is local browser state only and is not an analytics identifier.

Plausible configuration is deployment-controlled (`METEONEXA_PLAUSIBLE_ENABLED`, `METEONEXA_PLAUSIBLE_DOMAIN`). Browser CSP keeps `script-src 'self'`; only `connect-src` is extended to the fixed Plausible origin. Diagnostics reports configuration state without generating synthetic analytics events.

## Runtime state and database boundaries

The frontend has one authoritative mutable runtime state, created by `modules/esm/core/runtime-state.mjs`. `modules/esm/core/store.mjs` attaches to that object and exposes immutable projected domain views/events; it no longer keeps a mirrored legacy snapshot or requires explicit synchronization calls. Infrastructure helpers that do not own business state, such as weather/location formatting and tooltips, live in dedicated core modules rather than in `app.js`.
...
`modules/esm/bootstrap.mjs` is the native `type="module"` entry point. It resolves fingerprinted module URLs through the no-cache first-party asset manifest, installs the ESM boundaries/core, installs the ESM domains, loads the remaining classic application shells in their required order, and installs post-app ESM features at the matching lifecycle points. `app.js`, `advanced.js`, and the Suite date-picker bootstrap are safe whether they load before or after `DOMContentLoaded`.
...
MapLibre is an exception to the application-owned `dist/` pipeline because it is a third-party runtime dependency. The same-origin `vendor-asset.php` gateway resolves MapLibre only from a version-pinned npm package whose complete tarball must match a hardcoded SHA-512 integrity value before any executable entry is cached or served. Remote GeoJSON datasets use the same gateway as data only and are structurally validated rather than executed.

## Privacy and external services

Precise location is processed only where required for weather/route functionality. Browser-to-provider fan-out is avoided for the multi-model forecast path. Account, device and session data are kept out of public accuracy responses and aggregate Product Metrics. AI provider payloads are minimized and exclude sampled route coordinates.
...
P3 phase 3 treats `app.js` and `suite.js` as composition shells rather than permanent homes for reusable implementation. `visualization.mjs` owns the chart interaction/rendering engine, weather canvas effects and trend/motion chart primitives; `app.js` injects only the runtime context required by that service. `suite-support.mjs` owns Suite's shared API/storage/network/locale/date/geospatial support and is instantiated by `suite.js` with Runtime API and security dependencies.
...
P3 phase 4 moves forecast/history, model-intelligence, device/session lifecycle and global app lifecycle out of `app.js`, and moves the second Suite integration controller out of `suite.js`. The authoritative modules are `forecast-history.mjs`, `model-intelligence.mjs`, `device-sessions.mjs`, `app-lifecycle.mjs` and `suite-integrations.mjs`.

`bootstrap.mjs` installs the first four as pre-app services and installs `suiteIntegrations` after `suite.js` has published the Suite, Advanced and date-picker services. The latter therefore consumes late-bound dependencies through the same declared DI graph rather than relying on named globals or script-order side effects. Shared Suite transport/formatting remains in `suite-support.mjs`; integration requests preserve structured API errors (`code`, `payload`, HTTP status) and the former integration timeout contract.

The service graph now contains 26 ESM modules and 29 provided services. `qa/p3_shell_decomposition_smoke.mjs` enforces shell budgets of 5,100 lines for `app.js` and 2,400 lines for `suite.js` and rejects reintroduction of the extracted implementations. Current shell sizes are approximately 4,940 and 2,307 lines respectively. The P3 phase-4 shell revision was `p3-domain-decomposition-4`; the current RC revision is `rc1-runtimefix-12-lifecycle-guest`.


## P3 final — thin classic shells and explicit UI ownership

P3 closes with `app-utilities.mjs` owning weather sharing plus bug-report/media workflows and `suite-assistant.mjs` owning the Suite local/AI assistant, Copilot evidence rendering and assistant session UI. `app.js` and `suite.js` are now composition shells with enforced budgets of 4,700 and 1,400 lines respectively (current sizes about 4,637 and 1,343).
...
Product Metrics accepts a closed event allowlist and aggregate counters only. The client submits metrics without credentials, and the endpoint rejects user/device/location/prompt dimensions. Public Local Accuracy returns thresholded aggregate statistics only and does not expose contributor identities or counts below the publication threshold.

### Plausible traffic analytics

Plausible is used only for aggregate traffic/pageview analytics. MeteoNexa does not execute Plausible JavaScript: the local fingerprinted analytics module sends a minimal `pageview` payload to the fixed Plausible Events API origin. The route is generated from a closed navigation allowlist and strips selected location, coordinates, account/device IDs, searches, AI text, bug-report bodies and route samples. Initial acquisition parameters are individually allowlisted and external referrers are reduced to their origin.

The request uses `credentials: omit`, analytics failures are non-blocking, and no application session token is shared with Plausible. GPC/DNT and the local opt-out disable dispatch. The opt-out key is a preference only, not a visitor identifier. Product Metrics remains a separate first-party aggregate system.
...
The packaged QA suite validates current security/privacy/runtime invariants only. Test names are semantic rather than tied to previous application releases. The suite includes syntax/lint, database integrity/parity, authentication/security smoke tests, provider/model consistency, Radar probation, nowcast/decision regressions, Product Metrics minimization, Watch My Plan safeguards, AI orchestration payload minimization and asset fingerprint integrity.
...
- RC: bug `visualization`/Radar corretto con DI esplicita e API restituita ad `app.js`.
...
- analytics resta Plausible cookieless con opt-out/GPC-DNT come da contratto.
...
- facade stabili idratate dal controller applicativo dopo il caricamento di `app.js`;
...
Durante il controllo runtime pre-release è emerso che `visualization.mjs` conservava un riferimento implicito a `SERVICES` e costruiva le funzioni Radar senza restituirle ad `app.js`. La sintassi era valida e i vecchi smoke test non lo rilevavano, ma in browser poteva causare `ReferenceError` nel percorso Radar/bootstrap.
...
- Suite-only ESM rimossi dal critical path pre-`app.js`: circa **499,8 KB → 421,2 KB** sorgente pre-app.
- CSS render-blocking: circa **618 KB → 420 KB** sorgente, più MapLibre CSS rimosso dal percorso bloccante.
- `app.js` viene preloaded mentre Core/ESM vengono installati.
...
3. generare/committare `package-lock.json` quando il registry npm è disponibile (raccomandato per riproducibilità supply-chain);
...
`styles.css` and `suite.css` are generated compatibility artifacts. Edit the ordered partials under `styles/main/` and `styles/suite/`, then run:
