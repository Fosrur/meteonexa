...
for (const file of files) {
    ...
    if (check.status !== 0) fail(`syntax ${relative(ROOT, file)}`);
}
...
if (unexpectedAppGlobals.length) fail(`app.js bypasses service registry: ${unexpectedAppGlobals.join(', ')}`);
else pass('app.js service dependencies go through MeteoNexaServices');

for (const name of ['advanced.js', 'suite.js']) {
...
for (const name of ['advanced.js','suite.js','weather-intelligence.js','product-metrics.js','custom-controls.js']) {
    const source = sources.get(name) || '';
...
const advanced = sources.get('advanced.js') || '';
if (!/const APP_RUNTIME =/.test(advanced)) fail('advanced.js has no explicit runtime binding');
if (!/const APP_RUNTIME =/.test(sources.get('suite.js') || '')) fail('suite.js has no explicit runtime binding');
...
if (!/src=["']asset-manifest\.js["']/.test(indexHtml) || !bootstrapPublic || !indexHtml.includes(`type="module" src="${bootstrapPublic}"`)) fail('index.html missing native ESM bootstrap');
