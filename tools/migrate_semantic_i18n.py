#!/usr/bin/env python3
"""One-shot migration from opaque ui/code hashes to readable semantic i18n keys.
...
MAP_PATH = ROOT / 'api/install/i18n-key-map-20.1.json'
OPAQUE = re.compile(r'^(?:ui|code)\.[0-9a-f]{12,}$')
...
def source_priority(path: Path) -> tuple[int, str]:
    ...
    if rel.startswith('modules/esm/core/'):
        return (2, rel)
    if path.name in {'js/app.js','js/suite.js','js/advanced.js','js/weather-intelligence.js','js/custom-controls.js'}:
        return (3, rel)
    if path.suffix == '.html':
        return (4, rel)
    return (5, rel)
...
def rewrite_seed(mapping: dict[str, str]) -> None:
    ...
    seed['version'] = '20.1-semantic-i18n-v1'
...
def rewrite_baseline(mapping: dict[str, str]) -> None:
    with sqlite3.connect(DB) as connection:
        ...
        connection.execute("UPDATE app_metadata SET meta_value=?, updated_at=datetime('now') WHERE meta_key='translation_seed_version'", ('20.1-semantic-i18n-v1',))
        connection.commit()
...
def main() -> None:
    ...
    if not mapping:
        print('Semantic i18n migration: no opaque keys remain.')
        return
    MAP_PATH.write_text(json.dumps({'version':'20.1-semantic-i18n-v1','count':len(mapping),'mapping':mapping}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    ...
    print(f'Semantic i18n migration: {len(mapping)} keys migrated.')
