...
and service-worker caching resolves logical names through js/asset-manifest.js.
"""
...
'js/app.js','js/advanced.js','js/suite.js','js/weather-intelligence.js','js/privacy-context.js','js/page-i18n.js',
    'js/protected-page.js','js/offline.js','css/error.css','js/error.js',
...
LOGICAL_ALIASES={
    'intelligence-decision.css':'css/decision-timeline.css',
    'intelligence-watch.css':'css/watch-plan.css',
...
def reset_old_refs(previous:dict[str,str])->None:
    ...
    for name in ROOT_HTML:
        ...
        for logical in ASSETS:
            if logical.startswith(('install/','diagnostics/','qa/')) or logical in ('css/error.css','js/error.js'):
                continue
            rel=Path(logical)
...
def reset_aux_refs(previous:dict[str,str])->None:
    ...
    if p.exists():
        ...
        for logical in ('diagnostics/diagnostics.css','diagnostics/diagnostics.js'):
            ...
            if target:
                clean=str(target).removeprefix('./')
                s=s.replace('../'+clean,Path(logical).name)
        for logical in ('js/security-runtime.js','js/i18n-runtime.js'):
            target=previous.get(logical)
            if target:
                clean=str(target).removeprefix('./')
    ...
    if p.exists():
        s=p.read_text(encoding='utf-8')
        for logical in ('css/error.css','js/error.js'):
            target=previous.get(logical)
            if target:
                clean=str(target).removeprefix('./')
                s=s.replace('/'+clean,'/'+logical)
            rel=Path(logical)
            s=re.sub(r'/(?:[^\"\']*/)?dist/'+re.escape(rel.parent.as_posix())+'/'+re.escape(rel.stem)+r'\.[a-f0-9]{12}'+re.escape(rel.suffix),'/'+logical,s,flags=re.I)
        p.write_text(s,encoding='utf-8')
...
def rewrite_html(manifest:dict[str,str])->None:
    for name in ROOT_HTML:
        ...
        for logical,target in manifest.items():
            ...
            if logical.startswith(('install/','diagnostics/','qa/')) or logical in ('css/error.css','js/error.js'): continue
...
def rewrite_aux(manifest:dict[str,str])->None:
    ...
    if p.exists():
        ...
        replacements={
            ...
            '../js/security-runtime.js':'../'+manifest['js/security-runtime.js'].removeprefix('./'),
            '../js/i18n-runtime.js':'../'+manifest['js/i18n-runtime.js'].removeprefix('./'),
        }
    ...
    if p.exists():
        s=p.read_text(encoding='utf-8')
        s=re.sub(r'/css/error\.css(?:\?v=[^"\']+)?','/'+manifest['css/error.css'].removeprefix('./'),s)
        s=re.sub(r'/js/error\.js(?:\?v=[^"\']+)?','/'+manifest['js/error.js'].removeprefix('./'),s)
        p.write_text(s,encoding='utf-8')
...
def main()->None:
    ...
    sw={k:('./'+v.removeprefix('./')) for k,v in manifest.items() if not k.startswith(('install/','diagnostics/','qa/')) and k not in ('css/error.css','js/error.js')}
