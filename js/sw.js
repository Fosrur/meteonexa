'use strict';
const BUILD = '20.1';
const SHELL_REVISION = '20.1-final-04-assistant-runtime-radar';
...
function swScope() { return new URL(self.registration?.scope || '/', self.location.origin); }
const appUrl = value => new URL(String(value || '').replace(/^\.\//, ''), swScope()).href;
const asset = logical => appUrl(ASSET_MANIFEST[logical] || `${logical}?v=${BUILD}`);
const CRITICAL_SHELL = [
    appUrl(''), appUrl('index.html'),
    asset('css/styles.css'), asset('css/advanced.css'), asset('css/light-theme.css'),
    asset('js/boot-clock.js'), asset('js/i18n-runtime.js'), asset('js/config.js'), asset('js/security-runtime.js'), asset('js/custom-controls.js'),
    ...
    asset('js/app.js'), asset('js/advanced.js'), asset('js/suite.js'), asset('js/weather-intelligence.js'),
    appUrl('offline.html'), appUrl('assets/i18n/it.json'),
    appUrl(`assets/logo.png?v=${BUILD}`), appUrl(`assets/logo-full.png?v=${BUILD}`),
    appUrl(`assets/icons/icon-192.png?v=${BUILD}`), appUrl(`assets/icons/favicon-32.png?v=${BUILD}`), appUrl(`assets/icons/favicon.ico?v=${BUILD}`)
];
const OPTIONAL_SHELL = [
    asset('css/suite.css'), asset('css/intelligence.css'), asset('css/decision-timeline.css'), asset('css/watch-plan.css'), asset('css/copilot.css'), asset('css/standalone.css'),
    appUrl('privacy.html'), appUrl('cookie-policy.html'), appUrl('robots.txt'), appUrl('sitemap.xml'),
    appUrl('assets/i18n/en.json'), appUrl('assets/i18n/fr.json'), appUrl('assets/i18n/es.json'), appUrl('assets/i18n/de.json'),
    asset('js/privacy-context.js'), asset('js/page-i18n.js'), asset('js/offline.js'), asset('js/protected-page.js'),
    appUrl('api/vendor-asset.php?asset=maplibre-css'), appUrl('api/vendor-asset.php?asset=maplibre-js'), appUrl('api/vendor-asset.php?asset=italy-regions'), appUrl('api/vendor-asset.php?asset=italy-metros'),
    appUrl('assets/data/world-lines.json'),
    appUrl(`assets/icons/icon-512.png?v=${BUILD}`), appUrl(`assets/icons/maskable-512.png?v=${BUILD}`), appUrl(`assets/icons/apple-touch-icon.png?v=${BUILD}`)
];
...
async function loadCatalog(language = 'it') {
    ...
    try {
        ...
        const response = await fetch(appUrl(`api/i18n.php?${params}`), { cache: 'no-store', credentials: 'same-origin' });
        ...
        if (Object.keys(cachedCatalog).length) return cachedCatalog;
        try {
            const staticResponse = await fetch(appUrl(`assets/i18n/${locale}.json`), { cache: 'force-cache', credentials: 'omit' });
...
async function cacheShellEntries(entries, { priorityLanguage = '' } = {}) {
    ...
    const prioritized = [...entries].sort((a, b) => {
        const preferred = appUrl(`assets/i18n/${language}.json`);
...
self.addEventListener('fetch', event => { const request = event.request; if (request.method !== 'GET')
    ...
    const fallback = url.pathname.endsWith('/privacy.html') ? appUrl('privacy.html') : url.pathname.endsWith('/cookie-policy.html') ? appUrl('cookie-policy.html') : url.pathname.endsWith('/offline.html') ? appUrl('offline.html') : appUrl('index.html');
...
async function pullServerNotification(config) {
    ...
    try {
        const response = await fetch(appUrl('api/push/pending.php'), {
        ...
        await self.registration.showNotification(title, {
            body,
            icon: appUrl(`assets/icons/icon-192.png?v=${BUILD}`),
            badge: appUrl(`assets/icons/favicon-32.png?v=${BUILD}`),
            tag,
            renotify: false,
...
async function backgroundWeatherCheck() {
    ...
    await self.registration.showNotification(notice.title, { body: notice.body, icon: appUrl(`assets/icons/icon-192.png?v=${BUILD}`), badge: appUrl(`assets/icons/favicon-32.png?v=${BUILD}`), tag: `meteonexa-${notice.key}`, renotify: false, data: { url: safeTargetUrl(notice.url) }, vibrate: [120, 80, 120] });
}
