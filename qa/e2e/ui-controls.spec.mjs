...
test.describe('MeteoNexa custom controls and loaders', () => {
  ...
  test('radar layer controls preserve their hit targets after bootstrap', async ({ page }) => {
    test.setTimeout(45000);
    const radarNav = page.locator('[data-page="radar"]').first();
    ...
    for (const selector of ['[data-advanced-radar-layer="radar"]','[data-advanced-radar-layer="forecast"]','[data-advanced-radar-layer="satellite"]','[data-advanced-radar-layer="lightning"]']) {
      const control = page.locator(selector);
      await control.scrollIntoViewIfNeeded();
      await expect(control).toBeVisible();
      // Playwright's trial click is the browser-level hit-target/actionability
