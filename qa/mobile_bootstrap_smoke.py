...
ok('setTimeout(clearStaleOperationLoader, 12000)' in i18n,'i18n watchdog must only clear stale operation loader state')
ok('setTimeout(releaseBootUi, 12000)' not in i18n,'i18n watchdog can still reveal login/app before auth reconciliation')

app=text('js/app.js')
watchdog_start=app.find('const bootDeadline = setTimeout')
watchdog_end=app.find('    try {',watchdog_start)
watchdog=app[watchdog_start:watchdog_end] if watchdog_start>=0 and watchdog_end>watchdog_start else ''
ok(watchdog_start>=0 and watchdog_end>watchdog_start,'application boot watchdog block missing')
ok("classList.remove('app-boot-pending')" not in watchdog,'application boot watchdog can release the auth visibility gate early')
ok('recoverRootViewAfterBootFailure' not in watchdog,'application boot watchdog can choose login/app before auth reconciliation completes')
ok('BOOT_SLOW_PENDING_AUTH_RECONCILIATION' in watchdog,'slow authenticated boot diagnostic marker missing')
...
ok("document.dispatchEvent(new CustomEvent('meteonexa:ready'))" in app, 'legacy app-ready event is no longer emitted directly by app bootstrap')
ok('__METEONEXA_ESM_BOOTSTRAP_READY__' not in app, 'app-ready was incorrectly coupled to the deferred ESM bootstrap')
bootstrap=text('modules/esm/bootstrap.mjs')
ok("globalThis.__METEONEXA_INTERACTIVE_READY__ = true" in bootstrap and "meteonexa:interactive-ready" in bootstrap, 'full interaction layer does not publish its separate readiness contract')
helpers=text('qa/e2e/test-helpers.mjs')
ok('waitForMeteoNexaInteractiveReady' in helpers and "meteonexa:interactive-ready" in helpers, 'browser helper has no explicit interactive-ready gate')
assistant_e2e=text('qa/e2e/auth-guest.spec.mjs')
ok(assistant_e2e.count('waitForMeteoNexaInteractiveReady(page)') >= 2, 'Assistant browser tests are not gated on full interaction readiness')
...
ok("SHELL_REVISION = '20.1.1-maintenance-active-session'" in sw,'service worker shell revision is not the current 20.1 P2 final ESM-services revision')
