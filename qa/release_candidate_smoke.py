...
if expected_suite.intersection(pre_app):
    fail('suite-only ESM modules still block app.js')
...
if "preloadClassic('app.js')" not in bootstrap:
    fail('app.js preload missing')

expected_deferred = {'suite.css','intelligence.css','decision-timeline.css','watch-plan.css','copilot.css'}
...
blocking_css = ['styles.css','advanced.css','light-theme.css']
...
if 'vendor-asset.php?asset=maplibre-css' in index:
    fail('MapLibre CSS is still render-blocking in index.html')
if not re.search(r'<script[^>]+defer[^>]+src="vendor-asset\.php\?asset=maplibre-js"', index):
...
# must publish stable facades synchronously before app.js creates the controller.
