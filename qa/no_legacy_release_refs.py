...
errors=[]
for p in root.rglob('*'):
 if not p.is_file() or p.name=='readme.md' or p.suffix.lower() in ignore_ext or 'dist' in p.parts: continue
 try:s=p.read_text(encoding='utf-8')
 except Exception:continue
 for n,line in enumerate(s.splitlines(),1):
  if any(rx.search(line) for rx in patterns): errors.append(f'{p.relative_to(root)}:{n}: {line[:180]}')
for p in root.rglob('*'):
 if p.is_file() and p.name!='readme.md' and re.search(r'(?i)(?:release[_-]?(?:17|18|19)\d|_(?:17|18|19)\d{3,}|v(?:17|18|19)\d{2,})',p.name): errors.append('filename: '+str(p.relative_to(root)))
