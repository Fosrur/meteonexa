...
if len(keys) != 4645 or locales != ['de','en','es','fr','it']:
    fail(f'unexpected translation catalog shape: {len(keys)} keys / {locales}')
if len(rows) != 23225: fail(f'unexpected translation row count: {len(rows)}')
...
for locale in locales:
    ...
    if len(static_keys) != 4645 or opaque(static_keys): fail(f'hash-like/missing keys in static {locale} catalog')
