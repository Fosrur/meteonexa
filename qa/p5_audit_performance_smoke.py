...
for token in ['privacy-controller-name','privacy-controller-address','privacy-dpo-email','privacy-groq-link']:
...
if "SHELL_REVISION = '20.1-final-04-assistant-runtime-radar'" not in sw: fail('service-worker revision not bumped')
...
if len({row['text_key'] for row in seed.get('rows',[])})!=4645: fail('translation key count mismatch')
