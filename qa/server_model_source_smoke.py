...
files={name:(root/name).read_text(encoding="utf-8") for name in ["app.js","advanced.js","suite.js","modules/esm/domains/suite-support.mjs","config.js"]}
...
if "loadForecastFusion({ force })" not in files["app.js"]: err.append("app Intelligence does not reuse server fusion")
if "loadForecastFusion({ force })" not in files["advanced.js"]: err.append("Advanced does not reuse server fusion")
