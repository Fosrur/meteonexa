...
(() => {
    ...
    const DEFAULT_VISIBILITY = Object.freeze({
        'feature.assistant': { guest: false, authenticated: true },
    ...
    const applyPrivacyContext = ({ authenticated, verified, features = {}, services = {}, contact = {} }) => {
        ...
        const siteUrl = String(contact?.siteUrl || 'https://meteonexa.com/').trim();
        ...
        const controllerWarning = document.getElementById('privacy-controller-warning');
        ...
        if (controllerWarning) controllerWarning.hidden = legalConfigured;
        if (controllerSite) {
            try {
                const parsed = new URL(siteUrl, location.href);
                controllerSite.href = parsed.protocol === 'https:' ? parsed.href : 'https://meteonexa.com/';
                controllerSite.textContent = parsed.protocol === 'https:' ? parsed.host : 'meteonexa.com';
            } catch {
                controllerSite.href = 'https://meteonexa.com/';
                controllerSite.textContent = 'meteonexa.com';
            }
        }
