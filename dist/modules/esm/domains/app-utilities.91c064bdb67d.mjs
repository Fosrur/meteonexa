...
export const dependencies = Object.freeze(['feedback', 'metrics']);
    ...
    provided.appUtilities = Object.freeze({
        create(context) {
            ...
            function addBugReportFiles(files) {
                ...
                if (accepted) renderBugReportMedia();
            }
            ...
            async function startBugVideoRecording() {
                ...
                try {
                    ...
                    bugRecorderStream = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: { ideal: 15, max: 30 } }, audio: true });
                    ...
                    bugRecorder = new MediaRecorder(bugRecorderStream, options);
                    bugRecorderChunks = [];
                    bugRecorder.addEventListener('dataavailable', event => { if (event.data?.size) bugRecorderChunks.push(event.data); });
                    bugRecorder.addEventListener('stop', () => {
                        const actualType = String(bugRecorder?.mimeType || mimeType || 'video/webm').split(';')[0];
                        const chunks = bugRecorderChunks.slice();
                    ...
                    const preview = $('#bug-recording-video');
                    if (preview) preview.srcObject = bugRecorderStream;
                    bugRecorderStream.getVideoTracks?.().forEach(track => track.addEventListener('ended', () => { if (bugRecorder?.state === 'recording') bugRecorder.stop(); }, { once: true }));
                    const panel = $('#bug-recording-panel');
                    if (panel) panel.hidden = false;
                    bugRecorderStartedAt = Date.now();
                    bugRecorder.start(500);
                    const syncTimer = () => {
                        ...
                        if (seconds >= BUG_MEDIA_LIMITS.recordSeconds && bugRecorder?.state === 'recording') bugRecorder.stop();
                    };
                    syncTimer();
            ...
            function stopBugVideoRecording({ discard = false } = {}) {
                ...
                if (bugRecorder?.state === 'recording') bugRecorder.stop();
                else resetBugRecorderUI();
            }
